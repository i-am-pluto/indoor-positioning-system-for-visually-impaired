...
def getCurrentCheckpoint(features,all_features, image_paths):
    # Get current checkpoint here
    global current_checkpoint
    global prev_checkpoint
    df = pd.read_csv('./labels.csv')
    predicted_checkpoint = int(df.loc[ df['file'].isin( img for img in find_most_similar_images(features,all_features, image_paths, top_k=10)), 'labels'].value_counts().sort_values(ascending=False).index[0] )   
...
def getInstruction():
    ...
    shortest_path = nx.dijkstra_path(G, source=current_checkpoint, target=dest_checkpoint, weight='weight')
    ...
    curr_coord = checkpoint_df.loc[checkpoint_df['labels'] == current_checkpoint, 'coordinates'].values[0].split('_')
    ...
    turn_angle = 0  # Default angle value
    if np.linalg.norm(final_dirr_vector) != 0 and np.linalg.norm(dirr_vector) != 0:
        # Calculate turn angle only when both vectors are not zero
        turn_angle = math.acos(final_dirr_vector.dot(dirr_vector) / (np.linalg.norm(final_dirr_vector) * np.linalg.norm(dirr_vector))) * 180 / math.pi
